...
export default function AI() {
  ...
  const {
    ...
    updateApiConfig,
    deleteApiConfig,
    saveAiSettings, // Get the saveAiSettings function
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      console.log("Saving settings:", settingsToSaveBackend);
      await saveAiSettings(settingsToSaveBackend); // Use the store's save function
      await Dialog.show({ title: t("ai.ok.saveSuccess"), type: "success" });
      await refreshSettings();
    } catch (err) {
      console.error(t("ai.error.saveFailure"), err);
      // The store's save function might handle its own errors or throw.
      // Adjust the error message based on how the store's save function behaves.
      // For now, a generic error message is used.
      await Dialog.show({ title: t("ai.error.saveFailure"), content: String(err), type: "error" });
    }
  };
  ...
  const handleSaveApiConfigForm = (savedConfig: ApiConfig) => {
    ...
    Dialog.show({
      ...
      content: t("ai.form.saveSuccessMessage", "Configuration '{{name}}' saved successfully.", {
  ...
  const handleDeleteApiConfig = async (configId: string) => {
    const configToDelete = settings.api_configs.find((c: ApiConfig) => c.id === configId);
