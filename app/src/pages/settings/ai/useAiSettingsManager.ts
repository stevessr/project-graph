...
export function useAiSettingsManager() {
  ...
  const {
    ...
    resetAiSettings: storeResetAiSettings,
  } = useAiSettingsStore();
  ...
  const fetchModels = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          t("ai.error.fetch.ModelsError", { status: response.status, statusText: response.statusText, errorBody }),
        );
      }
      ...
      if (modelsData && modelsData.data && Array.isArray(modelsData.data)) {
        ...
        console.warn(t("ai.error.parseFailure"), modelsData);
        setAvailableModels([]);
      }
    } catch (err) {
      console.error(t("ai.error.fetch.Failure"), err);
      setFetchModelsError(
        t("ai.error.fetch.FailureDetail", { message: err instanceof Error ? err.message : String(err) }),
      );
      setAvailableModels([]);
    } finally {
  ...
  const confirmAndResetAISettings = async () => {
    try {
      const confirmation = await Dialog.show({
        title: t("ai.reset.title"),
        content: t("ai.reset.message"),
        type: "warning",
        buttons: [
          { text: t("ai.operation.cancel"), color: "white" },
          { text: t("ai.operation.reset"), color: "red" },
        ],
      });
      if (confirmation.button === t("ai.operation.reset")) {
        await storeResetAiSettings();
        await storeLoadAiSettings(); // Reload settings using store action
        await Dialog.show({ title: t("ai.ok.resetSuccess"), type: "success" });
      }
    } catch (err) {
      console.error(t("ai.dialogError"), err);
      await Dialog.show({ title: t("ai.error.dialogError"), content: String(err), type: "error" });
    }
  };
