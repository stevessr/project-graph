...
export namespace SoundService {
  ...
  async function getAudioBufferByFilePath(filePath: string): Promise<AudioBuffer | null> {
    const ac = getAudioContext();
    if (!ac) {
      // AudioContext not available, so we can't decode audio.
      // console.warn(`SoundService: AudioContext not available. Cannot decode audio data for ${filePath}.`);
      return null;
    }
    ...
    try {
      ...
      console.error(`SoundService: Error decoding audio data for ${filePath}:`, error);
