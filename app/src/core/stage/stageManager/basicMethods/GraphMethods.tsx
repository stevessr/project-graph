...
export namespace GraphMethods {
  ...
  export function getRoots(node: ConnectableEntity): ConnectableEntity[] {
    ...
    while (reverseEdges[rootUUID] && !visited.has(rootUUID)) {
      ...
      const parent = Stage.stageManager.getConnectableEntityByUUID(parentUUID);
      if (parent) {
        rootUUID = parentUUID;
      } else {
        break;
    ...
    const root = Stage.stageManager.getConnectableEntityByUUID(rootUUID);
    if (root) {
      return [root];
    } else {
      return [];
  ...
  export function getEdgesBetween(node1: ConnectableEntity, node2: ConnectableEntity): Edge[] {
    ...
    for (const edge of Stage.stageManager.getEdges()) {
