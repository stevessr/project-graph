...
impl Default for AiSettings {
    fn default() -> Self {
        ...
        let default_gemini_config = ApiConfig {
            ...
            model: "gemini-2.5-pro-preview-06-05".to_string(), // Or "gemini-pro"
            temperature: Some(0.7),
        ...
        // 6. oaipro Default
        let default_oaipro_config = ApiConfig {
            id: Uuid::new_v4().to_string(),
            name: "oaipro Default".to_string(),
            provider: "oaipro".to_string(),
            api_key: "".to_string(), // User must fill this
            base_url: "https://api.oaipro.com/v1".to_string(),
            model: "claude-sonnet-4-20250514".to_string(), // Example model, check Groq for available models
            temperature: Some(0.7),
            max_tokens: Some(8192),
            notes: Some(
                "Default configuration for Oaipro. Please add your API key from oaipro."
                    .to_string(),
            ),
        };
        default_api_configs.push(default_oaipro_config);

        // 6. openrouter Default
        let default_openrouter_config = ApiConfig {
            id: Uuid::new_v4().to_string(),
            name: "openrouter Default".to_string(),
            provider: "openrouter".to_string(),
            api_key: "".to_string(), // User must fill this
            base_url: "https://openrouter.ai/api/v1".to_string(),
            model: "deepseek/deepseek-r1-0528:free".to_string(), // Example model, check Groq for available models
            temperature: Some(0.7),
            max_tokens: Some(8192),
            notes: Some(
                "Default configuration for openrouter. Please add your API key from oaipro."
                    .to_string(),
            ),
        };
        default_api_configs.push(default_openrouter_config);
